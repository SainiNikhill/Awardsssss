...
const Navbar = () => {
  ...
  return (
    <div
      className={`fixed z-[999] w-full px-20 py-6 font-['Neue_Montreal'] flex justify-between items-center text-black 
      bg-white/10 backdrop-blur-md shadow-md transition-all duration-500 ${
      ...
      <div className="links flex gap-10">
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map(
          (item, index) => (
            <a
              key={index}
              className={`text-lg capitalize font-medium relative cursor-pointer 
              hover:text-red-500 transition-all duration-300 ${
                index === 4 ? "ml-32" : ""
              }`}
            >
              {item}
              <span className="absolute left-0 bottom-[-4px] w-0 h-[2px] bg-green-500 transition-all duration-300 hover:w-full"></span>
            </a>
          )
        )}
      </div>
